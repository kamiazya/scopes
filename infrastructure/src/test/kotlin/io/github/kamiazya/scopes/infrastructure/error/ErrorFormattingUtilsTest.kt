...
class ErrorFormattingUtilsTest : StringSpec({
    ...
    "getErrorMessage should format business rule violations properly" {
        ...
        val maxChildrenError = DomainError.ScopeBusinessRuleViolation.ScopeMaxChildrenExceeded(maxChildren = 10, actualChildren = 15)
        val duplicateError = DomainError.ScopeBusinessRuleViolation.ScopeDuplicateTitle("Duplicate Task", null)
        
        ErrorFormattingUtils.getErrorMessage(maxDepthError) shouldBe "Maximum hierarchy depth exceeded (max: 5, actual: 8)"
        ErrorFormattingUtils.getErrorMessage(maxChildrenError) shouldBe "Maximum children exceeded (max: 10, actual: 15)"
    ...
    "getValidationErrorMessage should handle all validation error types" {
        ...
        ErrorFormattingUtils.getValidationErrorMessage(descTooLong) shouldBe "Description too long (max: 1000, actual: 1500)"
        ErrorFormattingUtils.getValidationErrorMessage(invalidFormat) shouldBe "Invalid format for title, expected: String"
    }
    ...
    "getBusinessRuleViolationMessage should handle all business rule violations" {
        ...
        ErrorFormattingUtils.getBusinessRuleViolationMessage(maxDepth) shouldBe "Maximum hierarchy depth exceeded (max: 10, actual: 15)"
        ErrorFormattingUtils.getBusinessRuleViolationMessage(maxChildren) shouldBe "Maximum children exceeded (max: 20, actual: 25)"
