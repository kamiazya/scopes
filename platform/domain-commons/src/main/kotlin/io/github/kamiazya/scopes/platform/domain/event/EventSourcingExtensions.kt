 ...
 */
fun <A : AggregateRoot<A, ID, E>, ID : Identifier, E : DomainEvent> AggregateResult<A, ID, E>.withMetadata(metadata: EventMetadata): AggregateResult<A, ID, E> =
    copy(
 ...
 */
fun <A : AggregateRoot<A, ID, E>, ID : Identifier, E : DomainEvent> A.evolveWithPersisted(events: List<EventEnvelope.Persisted<E>>): A =
    events.fold(this) { aggregate, envelope ->
 ...
 */
fun <A : AggregateRoot<A, ID, E>, ID : Identifier, E : DomainEvent> A.evolveWithPending(events: List<EventEnvelope.Pending<E>>): A =
    events.fold(this) { aggregate, envelope ->
        aggregate.applyEvent(envelope.event)
    }
