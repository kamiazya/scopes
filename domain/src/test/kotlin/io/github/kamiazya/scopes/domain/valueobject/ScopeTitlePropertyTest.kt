            ...
            checkAll(emptyStringArb()) { title ->
                ...
                (
                    error is ScopeInputError.TitleError.Empty ||
                        error is ScopeInputError.TitleError.ContainsProhibitedCharacters
                    ) shouldBe true
            }
        }
        ...
        "scope titles should reject strings with newline characters" {
            checkAll(stringWithNewlinesArb()) { title ->
                ...
                if (trimmed.isNotEmpty() &&
                    !trimmed.contains('\n') &&
                    !trimmed.contains('\r') &&
                    trimmed.length <= ScopeTitle.MAX_LENGTH
                ) {
        ...
        "scope titles should handle unicode characters" {
            checkAll(
                Arb.string(1..100)
                    .filter {
                        it.trim().isNotEmpty() &&
                            it.trim().length <= ScopeTitle.MAX_LENGTH &&
                            !it.contains('\n') &&
                            !it.contains('\r')
                    },
            ) { title ->
