...
public abstract class Protocol(@PublishedApi internal val options: ProtocolOptions?) {
     ...
     */
    public inline fun <reified T : Request> setRequestHandler(method: Method, noinline block: suspend (T, RequestHandlerExtra) -> RequestResult?) {
        setRequestHandler(typeOf<T>(), method, block)
    }

    @PublishedApi
    internal fun <T : Request> setRequestHandler(requestType: KType, method: Method, block: suspend (T, RequestHandlerExtra) -> RequestResult?) {
