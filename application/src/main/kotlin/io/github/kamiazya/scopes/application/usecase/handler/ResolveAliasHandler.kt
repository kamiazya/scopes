 ...
 */
class ResolveAliasHandler(private val aliasManagementService: ScopeAliasManagementService) : UseCase<ResolveAliasQuery, ApplicationError, ResolveAliasResult> {

    override suspend operator fun invoke(input: ResolveAliasQuery): Either<ApplicationError, ResolveAliasResult> = AliasName.create(input.aliasName)
        .mapLeft { aliasError ->
            when (aliasError) {
