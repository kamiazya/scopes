 ...
 */
class ListAliasesForScopeHandler(private val aliasManagementService: ScopeAliasManagementService) : UseCase<ListAliasesForScopeQuery, ApplicationError, ListAliasesResult> {

    override suspend operator fun invoke(input: ListAliasesForScopeQuery): Either<ApplicationError, ListAliasesResult> = ScopeId.create(input.scopeId)
        .mapLeft { idError ->
            when (idError) {
