 ...
 */
data class AspectDefinitionWithRules(
    val definition: AspectDefinition,
    val rules: List<AspectRule> = emptyList()
) {
    /**
    ...
    fun validateRules(
        ...
        if (!definition.allowMultiple && values.size > 1) {
            return listOf(RuleValidationResult.Invalid(
     ...
     */
    fun isValidWithRules(
        value: AspectValue?,
        allAspects: Map<AspectKey, NonEmptyList<AspectValue>>
    ): Boolean = validateRules(value, allAspects).all { it is RuleValidationResult.Valid }
    ...
    fun isValidWithRules(
        values: List<AspectValue>,
        allAspects: Map<AspectKey, NonEmptyList<AspectValue>>
    ): Boolean = validateRules(values, allAspects).all { it is RuleValidationResult.Valid }
    ...
    fun getValidationMessages(
        ...
        validateRules(value, allAspects)
    ...
    fun getValidationMessages(
        ...
        validateRules(values, allAspects)
            .filterIsInstance<RuleValidationResult.Invalid>()
            .map { it.rule.message }
    ...
    fun compareValues(first: AspectValue, second: AspectValue): Int? = 
        definition.compareValues(first, second)
}