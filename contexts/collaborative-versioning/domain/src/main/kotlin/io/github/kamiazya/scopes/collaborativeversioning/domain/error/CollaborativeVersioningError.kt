...
sealed class VersionError : CollaborativeVersioningError() {
    data class VersionNotFound(val versionId: VersionId, override val occurredAt: Instant = SystemTimeProvider().now()) : VersionError()

    data class ResourceNotFound(val resourceId: ResourceId, override val occurredAt: Instant = SystemTimeProvider().now()) : VersionError()
    ...
    data class VersionAlreadyExists(val versionId: VersionId, override val occurredAt: Instant = SystemTimeProvider().now()) : VersionError()
}
...
sealed class ChangesetError : CollaborativeVersioningError() {
    data class ChangesetNotFound(val changesetId: ChangesetId, override val occurredAt: Instant = SystemTimeProvider().now()) : ChangesetError()

    data class InvalidChangeset(val changesetId: ChangesetId, val reason: String, override val occurredAt: Instant = SystemTimeProvider().now()) :
        ChangesetError()

    data class ChangesetAlreadyApplied(val changesetId: ChangesetId, val appliedAt: Instant, override val occurredAt: Instant = SystemTimeProvider().now()) :
        ChangesetError()
...
sealed class MergeError : CollaborativeVersioningError() {
    ...
    data class UnresolvedConflicts(val conflicts: List<ConflictDetail>, override val occurredAt: Instant = SystemTimeProvider().now()) : MergeError()
...
sealed class VersionNumberError : CollaborativeVersioningError() {
    data class InvalidValue(val providedValue: Int, val reason: String, override val occurredAt: Instant = SystemTimeProvider().now()) : VersionNumberError()
}
...
sealed class ResourceContentError : CollaborativeVersioningError() {
    data class EmptyContent(override val occurredAt: Instant = SystemTimeProvider().now()) : ResourceContentError()

    data class ContentTooLarge(val actualSize: Int, val maxSize: Int, override val occurredAt: Instant = SystemTimeProvider().now()) : ResourceContentError()

    data class InvalidJson(val reason: String, val cause: Throwable? = null, override val occurredAt: Instant = SystemTimeProvider().now()) :
...
sealed class TrackedResourceError : CollaborativeVersioningError() {
    data class NoSnapshots(val resourceId: ResourceId, override val occurredAt: Instant = SystemTimeProvider().now()) : TrackedResourceError()

    data class NoChangeHistory(val resourceId: ResourceId, override val occurredAt: Instant = SystemTimeProvider().now()) : TrackedResourceError()

    data class VersionNotFound(val resourceId: ResourceId, val versionNumber: VersionNumber, override val occurredAt: Instant = SystemTimeProvider().now()) :
...
sealed class TrackedResourceServiceError : CollaborativeVersioningError() {
    data class ResourceMismatch(val expected: ResourceId, val actual: ResourceId, override val occurredAt: Instant = SystemTimeProvider().now()) :
        TrackedResourceServiceError()

    data class InvalidContent(val reason: String, override val occurredAt: Instant = SystemTimeProvider().now()) : TrackedResourceServiceError()
}
...
sealed class AuthorError : CollaborativeVersioningError() {
    ...
    data class InvalidUserId(val providedValue: String, override val occurredAt: Instant = SystemTimeProvider().now()) : AuthorError()

    data class InvalidAgentId(val providedValue: String, override val occurredAt: Instant = SystemTimeProvider().now()) : AuthorError()
}
...
sealed class ChangeProposalError : CollaborativeVersioningError() {
    ...
    data class ProposedChangeNotFound(val proposedChangeId: String, override val occurredAt: Instant = SystemTimeProvider().now()) : ChangeProposalError()

    data class ParentCommentNotFound(val parentCommentId: String, override val occurredAt: Instant = SystemTimeProvider().now()) : ChangeProposalError()

    data class ProposalNotFound(val proposalId: ProposalId, override val occurredAt: Instant = SystemTimeProvider().now()) : ChangeProposalError()

    data class ConflictDetected(val conflicts: List<String>, override val occurredAt: Instant = SystemTimeProvider().now()) : ChangeProposalError()
}
...
sealed class SnapshotServiceError : CollaborativeVersioningError() {
    data class SnapshotNotFound(val resourceId: ResourceId, val snapshotId: SnapshotId, override val occurredAt: Instant = SystemTimeProvider().now()) :
        SnapshotServiceError()

    data class VersionNotFound(val resourceId: ResourceId, val versionNumber: VersionNumber, override val occurredAt: Instant = SystemTimeProvider().now()) :
        SnapshotServiceError()

    data class InvalidContent(val reason: String, override val occurredAt: Instant = SystemTimeProvider().now()) : SnapshotServiceError()

    data class ResourceMismatch(val expected: ResourceId, val actual: ResourceId, override val occurredAt: Instant = SystemTimeProvider().now()) :
        SnapshotServiceError()

    data class StorageLimitExceeded(val currentSize: Long, val maxSize: Long, override val occurredAt: Instant = SystemTimeProvider().now()) :
        SnapshotServiceError()

    data class SerializationError(val reason: String, val cause: Throwable? = null, override val occurredAt: Instant = SystemTimeProvider().now()) :
        SnapshotServiceError()

    data class DeserializationError(val reason: String, val cause: Throwable? = null, override val occurredAt: Instant = SystemTimeProvider().now()) :
        SnapshotServiceError()
    ...
    data class MetadataValidationError(val key: String, val value: String, val reason: String, override val occurredAt: Instant = SystemTimeProvider().now()) :
        SnapshotServiceError()
}
