...
sealed class AspectRule {
     ...
     */
    data class Range(val min: Double? = null, val max: Double? = null, override val message: String = "Value must be within allowed range") : AspectRule()

    /**
     * Pattern rule for text values using regex.
     */
    data class Pattern(val regex: String, override val message: String = "Value does not match required pattern") : AspectRule()

    /**
     * Required rule - value must be present when condition is met.
     */
    data class Required(val condition: AspectCondition, override val message: String = "This field is required") : AspectRule()

    /**
     * Forbidden rule - value must not be present when condition is met.
     */
    data class Forbidden(val condition: AspectCondition, override val message: String = "This field is not allowed") : AspectRule()
...
sealed class AspectCondition {
     ...
     */
    data class AspectEquals(val key: String, val value: String) : AspectCondition()

    /**
     * Check if another aspect is present.
     */
    data class AspectExists(val key: String) : AspectCondition()

    /**
     * Logical AND of multiple conditions.
     */
    data class And(val conditions: List<AspectCondition>) : AspectCondition()

    /**
     * Logical OR of multiple conditions.
     */
    data class Or(val conditions: List<AspectCondition>) : AspectCondition()

    /**
     * Logical NOT of a condition.
     */
    data class Not(val condition: AspectCondition) : AspectCondition()
}
 ...
 */
fun AspectRule.validate(value: AspectValue?, allAspects: Map<AspectKey, AspectValue>): RuleValidationResult {
...
private fun AspectRule.Required.validateRequired(value: AspectValue?, allAspects: Map<AspectKey, AspectValue>): Boolean {
    val conditionMet = condition.evaluate(allAspects)
    return !conditionMet || value != null
}

private fun AspectRule.Forbidden.validateForbidden(value: AspectValue?, allAspects: Map<AspectKey, AspectValue>): Boolean {
    val conditionMet = condition.evaluate(allAspects)
    return !conditionMet || value == null
}

fun AspectCondition.evaluate(aspects: Map<AspectKey, AspectValue>): Boolean = when (this) {
