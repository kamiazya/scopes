...
data class Aspects private constructor(private val map: Map<AspectKey, NonEmptyList<AspectValue>> = emptyMap()) {
     ...
     */
    fun findKeysWithValue(value: AspectValue): Set<AspectKey> = 
        map.filterValues { values -> values.contains(value) }.keys

    /**
     * Find all aspect keys that match a predicate on their values.
     */
    fun findKeysWhere(predicate: (NonEmptyList<AspectValue>) -> Boolean): Set<AspectKey> =
        map.filterValues(predicate).keys

    /**
     * Find all values across all aspects that match a predicate.
     */
    fun findValuesWhere(predicate: (AspectValue) -> Boolean): List<AspectValue> =
        map.values.flatMap { valueList -> valueList.filter(predicate) }

    /**
     * Check if any aspect key has a specific value.
     */
    fun hasValue(value: AspectValue): Boolean = 
        map.values.any { values -> values.contains(value) }

    /**
     * Check if any aspect matches a key-value combination.
     */
    fun hasAspect(key: AspectKey, value: AspectValue): Boolean =
        map[key]?.contains(value) ?: false

    /**
     * Get all distinct values across all aspects.
     */
    fun allValues(): Set<AspectValue> = 
        map.values.flatMap { it.toList() }.toSet()

    /**
     * Count total number of values across all aspects.
     */
    fun totalValueCount(): Int = 
        map.values.sumOf { it.size }

    /**
     * Filter aspects by key pattern (useful for querying related aspects).
     */
    fun filterByKeyPattern(pattern: Regex): Aspects = 
        copy(map = map.filterKeys { key -> pattern.matches(key.value) })

    /**
     * Filter aspects to only those containing values matching a pattern.
     */
    fun filterByValuePattern(pattern: Regex): Aspects = 
        copy(map = map.filterValues { values -> 
     ...
     */
    fun findMultiValuedAspects(): Aspects = 
        copy(map = map.filterValues { values -> values.size > 1 })

    /**
     * Find aspects that have only single values.
     */
    fun findSingleValuedAspects(): Aspects = 
        copy(map = map.filterValues { values -> values.size == 1 })
...
data class AspectQuery internal constructor(private val currentAspects: Aspects) {
     ...
     */
    fun whereKey(pattern: Regex): AspectQuery = 
        AspectQuery(currentAspects.filterByKeyPattern(pattern))

    /**
     * Filter by aspect key predicate.
     */
    fun whereKey(predicate: (AspectKey) -> Boolean): AspectQuery = 
        AspectQuery(currentAspects.filter { key, _ -> predicate(key) })

    /**
     * Filter by aspect value pattern.
     */
    fun whereValue(pattern: Regex): AspectQuery = 
        AspectQuery(currentAspects.filterByValuePattern(pattern))

    /**
     * Filter by aspect value predicate.
     */
    fun whereValue(predicate: (AspectValue) -> Boolean): AspectQuery = 
        AspectQuery(currentAspects.filter { _, values -> values.any(predicate) })

    /**
     * Filter to multi-valued aspects only.
     */
    fun multiValued(): AspectQuery = 
        AspectQuery(currentAspects.findMultiValuedAspects())

    /**
     * Filter to single-valued aspects only.
     */
    fun singleValued(): AspectQuery = 
        AspectQuery(currentAspects.findSingleValuedAspects())
