...
class ScopeAggregateTest : DescribeSpec({
        ...
        describe("Aspect events") {
            ...
            it("should apply ScopeAspectsUpdated event correctly") {
                ...
                val newAspects = Aspects.of(
                    AspectKey.create("status").getOrElse { throw RuntimeException(it.toString()) } to nonEmptyListOf(AspectValue.create("done").getOrElse { throw RuntimeException(it.toString()) })
                )
                val now = Clock.System.now()
...
private fun createTestAggregateWithAspects(): ScopeAggregate {
    ...
    val aspects = Aspects.of(
        AspectKey.create("priority").getOrElse { throw RuntimeException(it.toString()) } to nonEmptyListOf(AspectValue.create("high").getOrElse { throw RuntimeException(it.toString()) }),
        AspectKey.create("type").getOrElse { throw RuntimeException(it.toString()) } to nonEmptyListOf(AspectValue.create("feature").getOrElse { throw RuntimeException(it.toString()) })
    )
