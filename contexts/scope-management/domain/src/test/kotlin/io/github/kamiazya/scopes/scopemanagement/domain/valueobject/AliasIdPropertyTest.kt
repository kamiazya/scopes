...
class AliasIdPropertyTest :
    StringSpec({
        ...
        "AliasId should preserve ULID timestamp extraction" {
            checkAll(validUlidArb()) { ulid ->
                val aliasId = AliasId.create(ulid).getOrNull()!!

                // Should be able to extract timestamp from ULID
                val timestamp = ULID.getTimestamp(aliasId.value)
                timestamp shouldBe { it >= 0 }
            }
        }
