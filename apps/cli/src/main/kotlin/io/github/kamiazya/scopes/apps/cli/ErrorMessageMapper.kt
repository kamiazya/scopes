...
object ErrorMessageMapper {
    fun getMessage(error: ScopesError): String = when (error) {
        is ScopeError -> when (error) {
            ...
            is ScopeError.VersionMismatch -> "Version mismatch for scope ${error.scopeId.value}: expected ${error.expectedVersion} but was ${error.actualVersion}"
        }
        ...
        is ScopeHierarchyError -> when (error) {
            ...
            is ScopeHierarchyError.MaxChildrenExceeded -> "Maximum children exceeded for parent ${error.parentScopeId.value}: current ${error.currentChildrenCount}, maximum ${error.maximumChildren}"
        ...
        is PersistenceError -> when (error) {
            ...
            is PersistenceError.ConcurrencyConflict -> "Concurrency conflict for ${error.entityType} ${error.entityId}: expected version ${error.expectedVersion}, actual ${error.actualVersion}"
        }
        ...
        is ContextError -> when (error) {
            ...
            is ContextError.ContextNotFound -> "Context view not found${error.contextId?.let { ": $it" } ?: ""}${error.contextName?.let { " (name: $it)" } ?: ""}"
