 ...
 */
class NamingAndPackageConventionsTest : StringSpec({
    ...
    "domain packages should follow standard structure" {
        val requiredDomainPackages = listOf(
            "entity", "valueobject", "repository", "error"
        )
        val optionalDomainPackages = listOf(
            "service", "event", "model", "aggregate", "value"
        )
    ...
    "application packages should follow CQRS structure" {
        val expectedApplicationPackages = listOf(
            "command", "query", "handler", "dto", "error", "port"
        )
        ...
        boundedContexts.forEach { context ->
            Konsist
                ...
                .assertTrue { valueObject ->
                    ...
                    !valueObject.name.matches(Regex("^[A-Z]{2,}$")) && // Not all caps abbreviation
                    valueObject.name != "Id" && // Should be more specific like "UserId"
                    valueObject.name != "Name" && // Should be more specific like "UserName"
            ...
            Konsist
                ...
                .filter { clazz ->
                    ...
                    clazz.properties().any { prop ->
                        prop.name == "value" && prop.type.sourceType == "String"
                    } &&
    ...
    "similar concepts should have consistent names across contexts" {
        val commonConcepts = mapOf(
            ...
            "Event" to { name: String -> name.resideInPackage("..event..") }
        )
            ...
            boundedContexts.forEach { context ->
                Konsist
                    ...
                    .assertTrue { clazz ->
                        ...
                        when (concept) {
                            "Repository" -> clazz.hasInterfaceModifier || clazz.name.endsWith("RepositoryImpl")
                            "Service" -> !clazz.hasInterfaceModifier || clazz.hasObjectModifier
                            "Handler" -> clazz.functions().any { it.name == "handle" || it.name == "invoke" }
                            "Error" -> clazz.hasSealedModifier || clazz.hasDataModifier || clazz.hasObjectModifier
